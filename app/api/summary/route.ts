...
async function computePage(
  ...
  if (!Array.isArray(syms) || !total) {
    syms = ['BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT'].map(s => ({
      symbol: s,
      baseAsset: '',
      quoteAsset: 'USDT',
    }));
    ...
    Array.from({ length: CONC }).map(async () => {
      while (i < slice.length) {
        ...
        try {
          ...
          // --- безпечне звуження типів (жодних undefined у Row) ---
...
export async function GET(req: Request) {
  ...
  if (cached) {
    ...
    if (age <= fresh) {
      ...
      return NextResponse.json({
        ...
        meta: { ...cached.meta, cached: true, age, tookMs: Date.now() - t0 },
      });
    }
    if (age <= stale) {
      ...
      (async () => {
        try {
          ...
          const payload: Payload = {
            ...
            meta: { ...freshData.meta, revalidated: true },
          };
      ...
      return NextResponse.json({
        ...
        total: cached.total,
        nextOffset: cached.nextOffset,
        meta: { ...cached.meta, cached: true, stale: true, age, tookMs: Date.now() - t0 },
      });
    }
  ...
  return NextResponse.json({
    ...
    meta: { ...payload.meta, cached: false, tookMs: Date.now() - t0 },
  });
}
